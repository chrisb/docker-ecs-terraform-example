...
resource "aws_acm_certificate" "dns_cert" {
  domain_name       = aws_route53_record.dns_record.fqdn
...
resource "aws_route53_record" "cert_validation" {
  ...
  name            = tolist(aws_acm_certificate.dns_cert.domain_validation_options)[0].resource_record_name
  records         = [ tolist(aws_acm_certificate.dns_cert.domain_validation_options)[0].resource_record_value ]
  type            = tolist(aws_acm_certificate.dns_cert.domain_validation_options)[0].resource_record_type
